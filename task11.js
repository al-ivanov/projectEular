...
function maxMultiply() {
    const fileContent = fs.readFileSync("euler0011.txt", "utf8");
    let matrix = [];
    let resultArr = [];

    const rowCount = fileContent.split("\n");
    for (let i = 0; i < rowCount.length; i++) {
        const elementsArr = rowCount[i].split(' ');
        matrix[i] = [];
        for(let j = 0; j < elementsArr.length; j++) {
            matrix[i][j] = Number(elementsArr[j]);
        }
    }

    for(let i = 0; i < matrix.length; i++) {
        for(let j = 0; j < matrix[i].length - 3; j++) {
            resultArr.push(matrix[i][j] * matrix[i][j + 1] * matrix[i][j + 2] * matrix[i][j+3]);
        }
    }

    for(let i = 0; i < matrix.length - 3; i++) {
        for(let j = 0; j < matrix[i].length; j++) {
            resultArr.push(matrix[i][j] * matrix[i+1][j] * matrix[i+2][j] * matrix[i+3][j]);
        }
    }

    for(let i = 0; i < matrix.length - 3; i++) {
        for(let j = 0; j < matrix[i].length - 3; j++) {
            resultArr.push(matrix[i][j] * matrix[i+1][j+1] * matrix[i+2][j+2] * matrix[i+3][j+3]);
        }
    }
